...
function Resolve-IntrinsicAlias([int]$typeId) {
    $seen = @{}
    while ($typeId -gt 5) {
        if ($seen.ContainsKey($typeId) -or -not $aliasTargets.ContainsKey($typeId)) { throw "无法解析别名类型链: $typeId" }
        $seen[$typeId] = $true
...
Push-Location $root
try {
    & $StoryCompilerPath --game bg3 --game-data-path $vfs --no-packages --allow-type-coercion --mod $module --output $output --json
