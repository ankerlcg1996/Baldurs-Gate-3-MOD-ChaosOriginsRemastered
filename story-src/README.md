...
设置系统完全由原生 XAML、TutorialEvent、Story 数据库与一本角色专属设置手册组成，不依赖 NMCM、MCM 或 Script Extender。仅主机当前控制的混沌角色可在非战斗状态修改自己的设置；战斗中页面和手册操作均只读。
...
运行 `verify.ps1`、`compile-story.ps1`、`compile-resources.ps1` 后再执行 `build.ps1`。当前精确清单为 28 个 PAK 文件；静态检查、安装哈希一致和游戏内验收是三个不同结论。
