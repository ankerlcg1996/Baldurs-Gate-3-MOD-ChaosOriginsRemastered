# 混沌起源：重制版

![混沌起源：重制版海报](assets/branding/chaos-origins-remastered-poster.png)

挣脱既定命运，成为游离于所有身份之外的混沌之子。

“混沌起源：重制版”加入了一名全新的可玩起源角色。这个起源将所有官方起源角色身份与所有官方可选种族身份汇聚于一身。你不是在他们之间选择，而是同时成为他们。

你可以用一个角色进入不同起源人物与种族对应的剧情和对话，在冒险中承受不可预测的混沌变化，并通过真实的剧情选择获得不同命运带来的能力。

## 核心设计：拥抱随机

混沌起源的核心不是稳定、可预测的强大，而是让每场战斗都充满意外。

你的攻击可能被强化、延迟、转移或完全吞噬；受到伤害时可能获得祝福，也可能陷入眩晕、恐慌或易伤。混沌回响甚至可能伤害敌人，也可能治疗刚刚被你击中的目标。

这些随机结果并非单纯惩罚。坏运气会累积“混沌迷失”，并最终转化为可以主动使用的“混沌之力”。你承受的混乱越多，就越接近发动一次足以扭转战局的“混沌开天辟地”。

这名角色的玩法围绕三个选择展开：

- 是否接受随机结果带来的风险。
- 是否用混沌孤注押上命中率，换取更高伤害。
- 是否立即消耗混沌之力，还是留到真正绝望的时刻。

你无法完全控制混沌，但可以学习如何利用它。每一次攻击、受创和击杀，都可能把战斗推向全新的方向。

> 混沌不保证胜利。混沌只保证下一刻绝不会平凡。

## 所有角色，所有种族，一个起源

混沌既是阿斯代伦、盖尔、莱埃泽尔、影心、威尔、卡菈克与邪念，也是所有官方可选种族。不同身份不再彼此排斥，而是共同构成这个角色的完整存在。

这意味着你可以用同一个角色体验不同身份对应的剧情判定、特殊对话与命运选择。角色拥有所有种族的身份，但不会简单堆叠全部种族被动；本起源的重点是汇聚身份、拓展剧情可能，并用随机性重新塑造战斗。

## 基础能力

- 熟练所有简易武器、军用武器、护甲、盾牌与乐器。
- 十八项技能全部获得熟练与精通，并按正常熟练加值成长。
- 保留所有官方可选种族的身份判定。

## 七种起源身份

阿斯代伦、盖尔、莱埃泽尔、影心、威尔、卡菈克与邪念身份默认全部开启，并可在设置菜单中分别控制。

这些身份会影响对应的剧情判定与对话。部分身份还会提供初始能力：

- 阿斯代伦：吸血。
- 威尔：边境之刃。
- 卡菈克：汗液免疫与狂怒烈焰。

剧情能力不会按照人物等级自动发放，而是跟随你的真实剧情选择：

- 选择让阿斯代伦飞升，可获得吸血鬼飞升能力与蝙蝠变形。
- 推进盖尔的个人剧情，可获得耐色瑞尔魔球、幽影能力与成神力量。
- 完成威尔的相关抉择，可获得火焰护盾与召唤坎比翁。
- 修复卡菈克的炼狱引擎，可获得对应阶段的引擎强化。
- 接受邪念的命运，可获得杀戮者形态与一次性的“律令：死亡”。

已经获得的永久剧情奖励会继续保留。本MOD只赋予相应能力，不会复制剧情装备，也不会因为盖尔成神而把角色直接提升到20级。

## 混沌受创——主要被动能力

“混沌受创”是混沌起源的主要被动能力之一，始终伴随角色生效，无法关闭。

从你的一个回合开始到下个回合开始之间，第一次受到攻击或法术伤害时，将随机出现一种结果：

- 正面效果，例如祝福、加速、隐形、英勇或嗜血。
- 负面效果，例如眩晕、恐慌、沉默、倒伏、易伤或额外伤害。
- 潮湿。

敌人造成的负面结果会累积“混沌迷失”。迷失越深，转化为“混沌之力”的概率越高。击杀敌人也有机会获得混沌之力。

## 混沌开天辟地

每消耗3点混沌之力，可以发动一次“混沌开天辟地”：

- 恢复全部生命。
- 清除有害状态。
- 恢复行动、附赠动作与移动力。
- 暂时获得额外行动、自由施法、飞行与多项战斗强化。
- 获得一次威力惊人的混沌必杀。

未使用的混沌之力会持续保留，等待最关键的时刻。

## 混沌两仪——主要被动能力

“混沌两仪”是混沌起源的另一项主要被动能力，决定了角色最核心的攻击方式，始终生效，无法关闭。

你的直接伤害会被分成两部分：

- 一半正常结算。
- 另一半进入混沌轮盘。

轮盘可能改变伤害类型、延迟伤害、转移目标、扩大范围、分裂伤害、完全吞噬伤害，甚至将其强化至数倍。每次攻击都可能走向完全不同的结局。

## 其他混沌能力

- **混沌孤注：**牺牲命中率强化下一次攻击；只要命中，最终伤害翻倍。
- **混沌回响：**直接伤害可能追加力场伤害，也可能治疗仍然存活的目标。
- **混沌强袭：**12级解锁，每次短休一次，让关键攻势必然成功并大幅强化伤害。
- **自由设置：**可以调整混沌之力、混沌孤注、混沌回响及其他辅助机制；混沌受创与混沌两仪始终启用。

混沌既是力量，也是代价。你永远无法预知下一次攻击会毁灭敌人、拯救敌人，还是彻底改写战场。

## 制作与测试声明

本MOD由AI设计与制作。当前测试尚未发现影响正常游玩的严重问题，但仍可能存在未被发现的BUG。建议在体验前备份重要存档；如果遇到问题，欢迎反馈具体的触发条件与游戏表现。

---

# Chaos Origins: Remastered

Break free from predetermined fate and become a child of chaos beyond every fixed identity.

Chaos Origins: Remastered adds a new playable Origin character who unites every official Origin character identity and every official playable race in one body. You do not choose between them. You are all of them.

With one character, you can enter story paths and conversations tied to different Origins and races, endure unpredictable shifts of chaos, and gain powers from the destinies you choose to fulfil.

## Core Design: Embrace Randomness

Chaos Origins is not designed around stable, predictable power. Its defining feature is uncertainty.

Your attacks may be amplified, delayed, redirected, or erased. Taking damage may grant Bless, Haste, or Invisibility, but it may also leave you Stunned, Frightened, or Vulnerable. Chaos Echo may harm an enemy—or heal the target you just struck.

These random outcomes are more than simple punishment. Bad luck deepens Chaos Lost, which can eventually transform into Chaos Power. The more disorder you endure, the closer you come to unleashing Chaos Genesis and overturning the entire battle.

The character's playstyle revolves around three decisions:

- Whether to accept the risk of an unpredictable result.
- Whether to stake accuracy through Chaos All-In for greater damage.
- Whether to spend Chaos Power now or preserve it for a desperate moment.

You cannot fully control chaos, but you can learn to exploit it. Every attack, wound, and kill may send the battle in a new direction.

> Chaos does not promise victory. It only promises that the next moment will never be ordinary.

## Every Character, Every Race, One Origin

Chaos is Astarion, Gale, Lae'zel, Shadowheart, Wyll, Karlach, and the Dark Urge. Chaos is also every official playable race. These identities no longer exclude one another; together, they form a single Origin.

One character can experience story checks, special dialogue, and defining choices associated with many identities. The character carries every racial identity without simply stacking every racial passive. The design focuses on united identities, expanded story possibilities, and combat reshaped by randomness.

## Core Abilities

- Gain proficiency with all simple and martial weapons, armour, shields, and musical instruments.
- Gain proficiency and expertise in all eighteen skills, scaling with the normal proficiency bonus.
- Retain the identity checks of every official playable race.

## Seven Origin Identities

The identities of Astarion, Gale, Lae'zel, Shadowheart, Wyll, Karlach, and the Dark Urge are enabled by default and can be controlled separately through the settings menu.

These identities affect their corresponding dialogue and story checks. Some also grant immediate abilities:

- Astarion: Vampire Bite.
- Wyll: Blade of Frontiers.
- Karlach: Sweat Immunity and Rage Flames.

Story powers are earned through actual story outcomes rather than character level:

- Ascend Astarion to gain Vampire Ascendant powers and bat transformation.
- Advance Gale's story to claim the Netherese Orb, shadow powers, and godhood abilities.
- Complete Wyll's related choices to gain Fire Shield and Summon Cambion.
- Repair Karlach's Infernal Engine to receive its matching upgrade stage.
- Embrace the Dark Urge's fate to gain Slayer Form and a one-use Power Word Kill.

Permanently claimed story rewards remain yours. The mod grants the listed powers without copying story equipment, and Gale's godhood does not raise the character directly to level 20.

## Chaos Wound—Core Passive

Chaos Wound is one of the Origin's two core passive abilities. It remains active at all times and cannot be disabled.

Between the start of one of your turns and the start of the next, the first attack or spell that damages you draws a random result:

- A positive effect, such as Bless, Haste, Invisibility, Heroism, or Bloodlust.
- A negative effect, such as Stunned, Frightened, Silenced, Prone, Vulnerability, or bonus damage.
- Wet.

Negative results caused by enemies deepen Chaos Lost. The deeper it grows, the greater the chance that it transforms into Chaos Power. Kills can also generate Chaos Power.

## Chaos Genesis

Spend 3 Chaos Power to unleash Chaos Genesis:

- Restore all Hit Points.
- Remove harmful conditions.
- Recover Action, Bonus Action, and Movement.
- Temporarily gain extra actions, free spellcasting, flight, and powerful combat bonuses.
- Gain one devastating Chaos Finisher.

Unused Chaos Power remains until you choose the right moment to spend it.

## Chaos Duality—Core Passive

Chaos Duality is the Origin's other core passive ability. It defines the character's central offensive style, remains active at all times, and cannot be disabled.

Your direct damage is divided into two parts:

- Half resolves normally.
- Half enters the Chaos Roulette.

The roulette may change the damage type, delay the damage, redirect it, divide it among enemies, expand its area, erase it completely, or multiply it several times over. Every attack can reach a different conclusion.

## Other Chaos Powers

- **Chaos All-In:** Sacrifice accuracy on the next attack; if it still hits, its final damage is doubled.
- **Chaos Echo:** Direct damage may deal bonus Force damage or heal the surviving target.
- **Chaos Assault:** Unlocked at level 12 and usable once per Short Rest, it forces a decisive attack toward success and greatly increases its damage.
- **Custom Settings:** Adjust Chaos Power, Chaos All-In, Chaos Echo, and other supporting mechanics. Chaos Wound and Chaos Duality always remain active.

Chaos is both power and consequence. Your next attack may destroy an enemy, save one, or rewrite the entire battlefield.

## Creation and Testing Notice

This mod was designed and created by AI. Current testing has found no major issues that prevent normal play, but undiscovered bugs may still remain. Back up important saves before playing, and report the conditions and in-game behaviour if you encounter a problem.
