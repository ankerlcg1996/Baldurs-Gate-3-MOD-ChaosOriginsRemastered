...
设置按混沌角色分别保存。只有主机可以修改其当前控制的混沌角色；战斗中只允许查看。新角色、旧存档首次迁移和“恢复默认”均启用原有正式玩法设置；新增的二十项种族被动开关始终默认关闭。
...
使用手册后，玩家通过原生法术容器依次进入“常规机制”“种族被动”“起源身份”或“受击结果”。每个设置提供开启和关闭操作；与当前状态冲突的操作不可用。手册还提供“全部种族被动开启”“全部种族被动关闭”“全部身份开启”“全部身份关闭”和“恢复默认”。
...
系统为官方可选原生种族的每个种族被动提供独立开关。界面显示本地化被动名称，配置以官方被动 Stats ID 为唯一键；多个种族或亚种复用同名、同 Stats ID 的被动时只显示一项，不重复授予。审核后的去重清单共二十项：
...
二十项开关分别保存，默认全部关闭。种族被动页和设置手册均提供“一键全选”和“一键取消”；批量操作等价于依次执行二十个独立开关，但在同一个角色配置锁内完成，并在结束后统一同步界面。

系统使用 `DB_COS_RacialPassiveGranted(Character, Passive)` 记录设置系统实际补发的被动。开启某项时，角色已经拥有同一个 Stats ID 就不重复授予，也不写入授予账本；关闭该项时只移除账本内对应的被动。因此，角色由原生种族、其他能力或其他 MOD 预先获得的同名被动不受影响。“一键全选”同样跳过已存在的同名被动，“一键取消”同样只撤销设置系统实际授予的被动。种族被动开关不授予种族标签、种族主动技能或种族法术。
...
七起源身份开关不控制32种族身份标签、29项种族主动技能或种族法术。这些种族功能继续保持启用，避免关闭起源身份时误删种族能力。二十项官方种族被动仅由各自的独立开关控制。
...
Story 数据库是设置的唯一真实来源。每个角色分别保存四组数据：
...
注册混沌角色时，初始化过程只补充缺失的键。现有值不会被覆盖。八项原有机制、七项身份和十五项负面结果的默认值为 `1`；二十项种族被动的默认值均为 `0`。旧存档首次载入时也使用同一套默认值。

“恢复默认”把八项原有机制、七项身份和十五项负面结果改为 `1`，把二十项种族被动全部改为 `0`。重置时只撤销授予账本内的种族被动，不修改角色原本拥有的同名被动，也不修改混沌之力、迷失、击杀计数、永久剧情奖励或一次性奖励的消耗记录。
...
- 八项原有机制、二十项去重种族被动、七项身份和十五项负面结果在数据库、XAML、手册与本地化中一一对应。
- 每个 XAML 事件和手册技能只有一个 Story 处理器。
- 所有 UUID、状态机名称和资源路径唯一。
- PAK 不包含 SE、MCM 或 NMCM 文件。
- 配置初始化只补缺失键；二十项种族被动在新角色、旧存档首次迁移和恢复默认时均为 `0`。
- 单项关闭、一键取消或恢复默认只移除 `DB_COS_RacialPassiveGranted` 记录的对应被动。
- 一键全选和一键取消覆盖恰好二十个去重后的 Stats ID，不产生重复数据库行或重复授予。
- 恢复默认不修改运行计数和奖励账本。
...
- 新角色和旧存档首次迁移后，二十项种族被动均显示为关闭。
- 每项种族被动可以独立开启和关闭，其他十九项状态不受影响。
- 一键全选后二十项均显示开启；一键取消后二十项均显示关闭。
- 角色原本拥有的白名单同名被动在单项关闭、一键取消和恢复默认后仍然保留。
